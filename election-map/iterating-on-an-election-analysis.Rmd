...
We're going to fit a linear model which tries to predict the Two Party Vote Ratio $\frac{N_{clinton}}{N_{clinton}+N_{trump}} using population density.
...
My first thought after Jake's post was that the final model he fit was pretty simple. There are a few other variables it would be good to adjust for. What would the previous residual map look like if we controlled for factors like education, income, and age.

We're also going to deviate from the linear model we fit before. I think it would make more sense to fit a [logistic regression]() instead of a linear model, since the thing we want to predict is a proportion, not necessarily a continuous value.
...
results_pop_census_map %>%
    ...
    geom_polygon(colour = rgb(0.9,0.9,0.9, .1))+
    geom_polygon(data = map_data('state'), fill = NA, colour = 'white')+
    scale_fill_gradientn(values = rescale(c(-.6, -.3, -.1, 0, .1, .3, .6)),
                         colours = brewer_pal(palette = 'PiYG')(7),
                         limits = c(-1, 1),
                         name = 'Prediction Error (pink underestimates Trump, green underestimates Clinton)')+
    theme_minimal()+
