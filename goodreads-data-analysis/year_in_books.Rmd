...
```{r echo=FALSE}
books_read <- summary_by_date %>%
    ggplot(aes(date_read, cumu_books_read))+
    geom_line()+
    geom_point()+
    xlab("Date")+
    ylab("Cumulative Books Read")+
    ggtitle("Books Read in 2020")


pages_read <- summary_by_date %>%
