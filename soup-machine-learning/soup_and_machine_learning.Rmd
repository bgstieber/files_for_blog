...
Just looking at the [scatterplot matrix](https://www.itl.nist.gov/div898/handbook/eda/section3/scatterb.htm) above, we can see that trying to classify flowers into the setosa species is a bit of a toy problem (the red points are well-separated from the blue and green). Using this data, just by examining if the petal length of a flower is smaller than 2.45, we can determine if the flower is from the setosa species.
...
Feature engineering and selection is one of the most time consuming parts of the machine learning process. To keep this post brief, I'm going to go through just a few feature engineering and selection steps.
...
The procedure we used for variable selection is pretty straightforward, and suggested an interaction between the petal length and petal width variables, and petal length squared. It's generally a bad idea to include higher ordered terms without also including the lower order terms, so our final model will have three variables: petal width, petal length, and petal width * petal length interaction.
...
It looks like we can explore $\lambda$ a bit more, and at the same time improve our model fit.
...
Cross-validation suggests a rather small value of $\lambda$, indicating that the model's fit doesn't improve with a high degree of regularization. In the next section, we'll use two values of $\lambda$ to fit the model ($\lambda_1$ = `r lambda_1se_1`, $\lambda_2$ = `r lambda_2`), and then investigate the results. 

## Tasting the soup

After we fit the model (or make the soup), we have to determine how good it is. For this example, I'm going to use a simple method for determining the classification accuracy. Basically, I'm just going to check the % of time the classifier got the species correct. Using this metric implies that we're treating false positive and false negatives as equally bad errors. In most real world situations, this is not the case. 
...
In addition to the two models fit using regularization, I'm also going to fit models using _all_ of the variables we've created, as well as a simple model with only the original variables from the iris data set.
...
In the table above, we can see that the unregularized models resulted in better predictions on our test set. This shouldn't be too surprising, as this classification example is fairly contrived. In more realistic settings, we may see better predictive performance from models fit using regularization.
...
These situations have clear parallels to machine learning (maybe you got lucky and your prediction task is fairly easy or maybe all you need is a simple model or maybe you're an experience data scientist).
