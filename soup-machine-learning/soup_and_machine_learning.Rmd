...
Just looking at the paired scatter plot above, we can see that trying to classify flowers into the setosa species is a bit of a toy problem. Using this data, just by examining if the petal length of a flower is smaller than 2.45, we can determine if the flower is from the setosa species.
...
After we fit the model (or make the soup), we have to determine how good it is. For this example, I'm going to use a simple method for determining the classification accuracy. Basically, I'm just going to check the % of time the classifier got the species correct. 
