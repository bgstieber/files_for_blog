...
full_pca %>%
  ...
  ggplot(aes(PC1, PC2, colour = type), alpha = 0.7)+
  geom_point(size = 2)+
  scale_colour_manual(values = c('testing' = 'green',
                                 'training' = 'red') %>% rev,
                      labels = c('candidates', 'curated') %>% rev,
                      name = 'Song Type')+
  ggtitle('First Two Principal Components for Holiday Songs')
```

We can see that almost all of the songs from my holiday playlist have values for PC2 below zero. Let's look at the variable loadings for the first two principal components. These loadings describe the weights that are multiplied by the variables to calculate the respective principal components.
...
There are three variables which have positive loadings for PC2 (duration_ms, liveness, and energy). 
