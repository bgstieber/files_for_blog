...
There are three variables which have positive loadings for PC2 (duration_ms, liveness, and energy). The largest positive weight is for duration_ms, implying that I tend to prefer shorter holiday songs.
...
Our main purpose for building models is to find songs that are similar to the ones in my curated playlist. We're going to tackle this using a collection of data science strategies. We're going to build three types of models: a logistic regression using the first four principal components, a random forest model on the full data, and a nearest neighbors search to find minimum distances.
